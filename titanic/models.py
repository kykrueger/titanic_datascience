...
class MajorityVoteClassifier:
    ...
    This class contains the `fit` and `predict` methods that are
    compatible with the SciKit-Learn model classes.
    """
    def __init__(self):
    ...
    def predict(self, X):
        if self.majority_vote is None:
            raise ValueError("The majority vote classifier has to be\
...
def run_majority_vote(X_train, X_test, y_train, y_test):
    ...
    logging.info('The prediction accuracy with the majority vote\
            classifier is {:.1f}%'.format(accuracy * 100))
...
def run_logistic_regression(X_train, X_test, y_train, y_test):
    ...
    logging.info('The prediction accuracy with the ridge logistic \
            regression classifier is {:.1f}%'.format(accuracy * 100))
